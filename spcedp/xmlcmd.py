...
def _fromstring_no_dtd(xml_bytes: bytes) -> ET.Element:
    """Parse XML bytes to an element tree, refusing any DTD.

    Drives expat directly with a DOCTYPE handler that raises, so a DTD is
    rejected at the post-decode layer.  This catches a DTD declared in *any*
    encoding - e.g. a UTF-16 payload whose bytes carry no ASCII "<!DOCTYPE" -
    which a raw-byte substring check on the wire bytes cannot.  With no DTD
    permitted, no custom entities can be defined, so internal-entity expansion
    ("billion laughs") cannot occur; an external entity is never fetched (no
    ExternalEntityRefHandler is installed).  Any parse failure is surfaced as a
    ValueError so callers (Session.xml_command) map it to SpcProtocolError.
    """
    builder = ET.TreeBuilder()
    parser = expat.ParserCreate()

    def _reject_dtd(*_args: object) -> None:
        raise ValueError("XML reply contains a DTD; rejected")
    ...
    try:
        ...
        raise ValueError(f"malformed XML reply: {exc}") from exc
...
def parse_reply(xml_bytes: bytes) -> XmlReply:
    ...
    # the panel never sends a DTD, so reject any frame carrying one (see
    # _fromstring_no_dtd for why this is done at the parser layer, not by a
    # raw-byte substring check).
    root = _fromstring_no_dtd(xml_bytes)
