...
- Python 3.13 added to the test matrix; coverage reporting, `uv run --frozen`,
  and a lockfile-drift check in CI; `SECURITY.md`, `CONTRIBUTING.md` and
  Dependabot added; `cryptography>=43`.
- Version `0.2.0`, classifier "Beta". A stable `1.0.0` follows once the live
  campaign (`tests/live/`) is run against panel hardware.
